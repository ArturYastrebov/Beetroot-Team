...
async def start(message: types.Message):
    await bot.send_message(message.from_user.id, f"Hello: {message.from_user.id}",
                           reply_markup=subscribers_keyboard())

@dp.callback_query_handler(text='cd_btn_subscr')
async def command_btn_subscr(query: types.CallbackQuery):
    user_id = query.message.chat.id
    user_name = query.message.chat.username if query.message.chat.username \
                                               is not None else query.message.chat.full_name
