...
def delete_user_by_chat_id(db: Session, chat_id: int):
    db_user = get_user_by_chat_id(db, chat_id=str(chat_id))
    db.delete(db_user)
    db.commit()
    return db_user

# def delete_message_by_user_id(db: Session, user_id: int):
#     db_message = get_message_by_user_id(db, user_id=user_id)
#     print(db_message)
#     print(len(db_message))
