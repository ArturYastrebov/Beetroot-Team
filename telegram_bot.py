...
from async_parser.data_parser import format_str_command_manu, parser_items
from async_parser.formatter import formatter_manu, FORMATTER_DICT, save_csv
...
async def cmd_start(message: types.Message):
    await bot.send_photo(chat_id=message.from_user.id,
                         photo='https://upload.wikimedia.org/wikipedia/commons/thumb/c/c5/Ikea_logo.svg/'
...
@dp.message_handler(lambda message: message.text in str(list(COMMAND_DICT.keys())) and message.text != "0",
...
@dp.message_handler(lambda message: message.text,
                    state=ClientStatesGroup.sub_category)
...
@dp.message_handler(lambda message: message.text,
                    state=ClientStatesGroup.type_formatter)
